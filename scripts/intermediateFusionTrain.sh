...
#SBATCH --job-name=IntermediateFusionModelTrain
#SBATCH -e ./out/R-%x.%j.out
#SBATCH -o ./out/R-%x.%j.out
#SBATCH --open-mode=append

NOW=$( date '+%F-%H-%M-%S' )
JOB_NAME=IntermediateFusionModelTrain
...
srun -K\
  ...
  python -m main --root-dir "/netscratch/$USER/float32_dataset" --num-workers 1 --batch-size 64 --out-dir "/netscratch/$USER/dataset" --model-choice "intermediate_fusion" --train-countries "all_bands" --test-countries "all_bands" --clip true --lr 0.0443 --notes "Intermediate fusion: Trying with new learning rate 4.43E-02 (0.0443)"  
