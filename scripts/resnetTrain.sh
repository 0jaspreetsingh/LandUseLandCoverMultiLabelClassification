...
#SBATCH --job-name=ResnetModelTrain
#SBATCH -e ./out/R-%x.%j.out
#SBATCH -o ./out/R-%x.%j.out
#SBATCH --open-mode=append

NOW=$( date '+%F-%H-%M-%S' )
JOB_NAME=ResnetModelTrain
...
srun -K\
  ...
  python -m main --root-dir "/netscratch/$USER/float32_dataset" --num-workers 1 --batch-size 64 --out-dir "/netscratch/$USER/dataset" --model-choice "resnet" --train-countries "all_bands" --test-countries "all_bands" --lr 0.001 --notes "training all bands (resized) with resnet and learning rate 0.001"
