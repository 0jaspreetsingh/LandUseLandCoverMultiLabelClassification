...
#SBATCH --job-name=MergeModelTrain
#SBATCH -e ./out/R-%x.%j.out
#SBATCH -o ./out/R-%x.%j.out
#SBATCH --open-mode=append

NOW=$( date '+%F-%H-%M-%S' )
JOB_NAME=MergeModelTrain
...
srun -K\
  ...
  python -m main --root-dir "/netscratch/$USER/float32_dataset" --num-workers 1 --batch-size 64 --out-dir "/netscratch/$USER/dataset" --model-choice "merge" --train-countries "all_bands" --test-countries "all_bands" --clip true --notes "Esamble model"
