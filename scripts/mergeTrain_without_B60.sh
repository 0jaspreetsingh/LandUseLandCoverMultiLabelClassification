...
#SBATCH --job-name=MergeWithoutB60ModelTrain
#SBATCH -e ./out/R-%x.%j.out
#SBATCH -o ./out/R-%x.%j.out
#SBATCH --open-mode=append

NOW=$( date '+%F-%H-%M-%S' )
JOB_NAME=MergeWithoutB60ModelTrain
...
srun -K\
  ...
  python -m main --root-dir "/netscratch/$USER/float32_dataset" --num-workers 1 --batch-size 64 --out-dir "/netscratch/$USER/dataset" --model-choice "merge_without_B60" --train-countries "all_bands" --test-countries "all_bands" --clip true --lr 0.001
