...
class Model(nn.Module):
    def __init__(self,device, variant='efficientnet-b5', lbl_corr_vec=None, n_channels=3, n_classes=19):
        ...
        self.convReduction1 = nn.Sequential(
            nn.Conv2d(channel_sizes[0], 64, kernel_size=(2, 2), stride=(2, 2)),
            nn.BatchNorm2d(64),
            nn.ReLU()
        )
        self.convReduction2 = nn.Sequential(
            nn.Conv2d(channel_sizes[1] + 64, 128, kernel_size=(2, 2), stride=(2, 2)),
            nn.BatchNorm2d(128),
            nn.ReLU(),
    ...
    def forward(self, x):
        x = x.to(self.device)
        eps = self.model.extract_endpoints(x)
