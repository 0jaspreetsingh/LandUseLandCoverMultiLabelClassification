...
class EsambleModel(nn.Module):
    def __init__(self, device, variant='efficientnet-b5', lbl_corr_vec=None, n_classes=19):
        ...
        self.device = device
        variant_B10, variant_B20, variant_B60 = 'efficientnet-b5', 'efficientnet-b0', 'efficientnet-b0'
        ...
        self.b10_model = EfficientNet.from_name(variant_B10, 4)
        self.b20_model = EfficientNet.from_name(variant_B20, 6)
        # self.b60_model = Net()
        ...
        self.cbam = CBAMBlock(
            channel=channel_sizes_B10[2] + channel_sizes_B20[2], reduction=16, kernel_size=7)  # Kernel size is a
        # hyper-paramater
        self.register_buffer('lbl_corr_vec', lbl_corr_vec, persistent=False)
        self.lbl_layers = nn.Sequential(
            nn.BatchNorm1d(19*19 + channel_sizes_B10[2] + channel_sizes_B20[2]),
            nn.Linear(19*19 + channel_sizes_B10[2] + channel_sizes_B20[2], 64),
            nn.Linear(64, 128),
            nn.ReLU()
        )
        self.fc = nn.Linear(
            channel_sizes_B10[2] + channel_sizes_B20[2] + 128, n_classes)
    ...
    def forward(self, x):
        ...
        x = torch.concat((B10, B20), dim=1)
        x = self.cbam(x)
        x_pool = self.b10_model._avg_pooling(x).squeeze()
        lc_in = torch.concat(
        ...
        x = self.b10_model._dropout(x_pool).squeeze()
        x = self.fc(x)
        return x